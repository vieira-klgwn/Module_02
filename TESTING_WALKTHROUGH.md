...
This guide walks you through the **entire utility billing system** the way a real organization would use it — from first login to a paid bill and customer notification.

Use **Postman** (recommended) or **curl**. Keep a notepad (or Postman environment variables) to save IDs and tokens as you go.
...
Wait until you see the application started message. PostgreSQL must be running.

### 2. Open Postman

Create a collection called **Utility Billing** and an environment with these variables (you will fill them in as you go):

| Variable | You will set it when… |
|----------|----------------------|
| `baseUrl` | Now → `http://localhost:8080` |
| `adminToken` | Patrick logs in |
| `operatorToken` | Alice logs in |
| `financeToken` | David logs in |
| `customerToken` | Marie signs up |
| `customerId` | Marie's customer record is created |
| `waterMeterId` | Water meter is created |
| `elecMeterId` | Electricity meter is created |
| `waterReadingId` | Water reading is recorded |
| `elecReadingId` | Electricity reading is recorded |
| `waterBillId` | Water bill is generated |
| `waterBillRef` | Water bill is generated |
| `waterBillTotal` | Water bill is generated |

### 3. How to send the token in Postman

For every secured request:

1. Go to **Authorization** tab
2. Type: **Bearer Token**
3. Paste the token for the person you are acting as
...
## Final Checklist — Did the System Work?

Go through this list after completing the walkthrough:

### Authentication
- [ ] Admin, operator, and finance can log in with default credentials
- [ ] Customer can sign up without a token
- [ ] Protected endpoints reject requests with no token
- [ ] Logout invalidates the token

### Roles
- [ ] ADMIN manages users and tariffs
- [ ] OPERATOR creates readings only
- [ ] FINANCE generates bills, approves bills, records payments
- [ ] CUSTOMER can view but not create readings or payments

### WASAC & REG
- [ ] Water meter uses `WATER` type and water tariff
- [ ] Electricity meter uses `ELECTRICITY` type and tiered tariff

### Business logic
- [ ] Bill calculated from reading consumption
...
- [ ] Duplicate monthly reading is blocked
- [ ] Invalid reading (current ≤ previous) is blocked
- [ ] Inactive customer cannot receive new readings

### Notifications
- [ ] Notification created when bill is generated
- [ ] Notification created when payment is recorded
- [ ] Message format includes customer name, month/year, and amount

---

## Tips

1. **Use unique values** if re-running the walkthrough on the same database (new `nationalId`, `meterNumber`, email).
2. **Swagger UI** at `http://localhost:8080/swagger-ui/index.html` is useful for exploring endpoints, but use Postman for the full role-switching flow.
3. **Save IDs immediately** after each create response — later steps depend on them.
4. See `README.md` for the full endpoint reference and validation rules.

---

## Default Test Users (Quick Reference)

| Email | Password | Role |
|-------|----------|------|
| `admin@wasac.rw` | `Admin@1234` | ADMIN |
| `operator@wasac.rw` | `Operator@123` | OPERATOR |
